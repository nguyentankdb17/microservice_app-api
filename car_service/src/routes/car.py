...
async def create_car(car: schemas.CreateCar, db: Session = Depends(get_db), user: dict = Depends(require_admin)):
...
async def update_car(car_id: int, car: schemas.Car, db: Session = Depends(get_db), user: dict = Depends(require_admin)):
...
async def delete_car(request: Request, car_id: int, db: Session = Depends(get_db), user: dict = Depends(require_admin)):
    db_car = db.query(models.Car).filter(models.Car.id == car_id).first()
    if not db_car:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="car not found")
    db.delete(db_car)
